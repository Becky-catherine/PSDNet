...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim=None, cond_emb_dim=None, groups=8):
        ...
        # 新增：条件的 MLP
        self.cond_mlp = nn.Sequential(nn.SiLU(), nn.Linear(cond_emb_dim, dim_out * 2)) if exists(cond_emb_dim) else None
    ...
    def forward(self, x, time_emb=None, cond_emb=None):
        ...
        if (self.mlp is not None) or (self.cond_mlp is not None):
            t_embed = self.mlp(time_emb) if (self.mlp is not None and exists(time_emb)) else 0
            c_embed = self.cond_mlp(cond_emb) if (self.cond_mlp is not None and exists(cond_emb)) else 0
            fuse = t_embed + c_embed
            fuse = rearrange(fuse, 'b c -> b c 1 1')
            scale_shift = fuse.chunk(2, dim=1)

        h = self.block1(x, scale_shift=scale_shift)
        h = self.block2(h)
        return h + self.res_conv(x)
class CondPyramid(nn.Module):
    def __init__(self, in_ch, dims):  # dims: 与 UnetRes 的尺度列表一致 [init_dim, dim*1, dim*2, ...]
        super().__init__()
        self.stem = nn.Sequential(
            nn.Conv2d(in_ch, dims[0], 7, padding=3),
        ...
        for i in range(len(dims)-1):
            self.downs.append(nn.Sequential(
                ...
                nn.SiLU(),
                Downsample(dims[i], dims[i+1])  # 与主干对齐的降采样
            ))
        self.out_dims = dims
...
class Unet(nn.Module):
    def __init__(
        ...
        num_resolutions = len(in_out)     # ← 必须加这一句
        ...
        # ====== Multi-scale shallow supervision (after building self.ups) ======
        self.enable_ms_supervision = True              # turn on
        self.ms_scales = {0, 2}                        # choose stages to supervise
        self.ms_heads = nn.ModuleDict()
        
        reversed_in_out = list(reversed(in_out))
        for up_idx, (dim_in, dim_out) in enumerate(reversed_in_out):
            # 注意：此处用 dim_in（上采样后的通道）而不是 dim_out
            self.ms_heads[f'up_{up_idx}'] = nn.Conv2d(dim_in, self.out_dim, 1)
...
class UnetRes(nn.Module):
    def __init__(
        ...
        if self.share_encoder == 1:
            ...
            # ------------- in UnetRes.__init__ (share_encoder == 1) -------------
            # ===== [A] 判定是否有“条件张量” =====
            cond_in_ch = channels * (1 if condition else 0) + channels * (1 if input_condition else 0)
            self.has_cond = (cond_in_ch > 0)
            
            # dims / in_out 你已有
            # time_dim 你已有：time_dim = dim * 4
            
            # ===== [B] 条件金字塔，与主干的 dims 对齐 =====
            if self.has_cond:
                self.cond_encoder = CondPyramid(cond_in_ch, dims)  # 从 HxW 输出多尺度 feature
                # 每个尺度用 GAP -> 向量 -> 投到 time_dim，便于与 time_emb 相加
                self.to_cond_vec  = nn.ModuleList([
                    nn.Sequential(
                        nn.AdaptiveAvgPool2d(1),
                        Rearrange('b c 1 1 -> b c'),
                        nn.Linear(dims[i], time_dim)
                    ) for i in range(len(dims))
                ])
            else:
                self.cond_encoder = None
                self.to_cond_vec  = None
            
            # ===== [C] 所有 ResnetBlock 加上 cond_emb_dim=time_dim =====
            self.downs = nn.ModuleList([])
            for ind, (dim_in, dim_out) in enumerate(in_out):
                is_last = ind >= (num_resolutions - 1)   # ← 现在 num_resolutions 已定义
                self.downs.append(nn.ModuleList([
                    ResnetBlock(dim_in, dim_in, time_emb_dim=time_dim, cond_emb_dim=(time_dim if self.has_cond else None)),
                    ResnetBlock(dim_in, dim_in, time_emb_dim=time_dim, cond_emb_dim=(time_dim if self.has_cond else None)),
                    Residual(PreNorm(dim_in, LinearAttention(dim_in))),
            ...
            mid_dim = dims[-1]
            self.mid_block1 = ResnetBlock(mid_dim, mid_dim, time_emb_dim=time_dim, cond_emb_dim=(time_dim if self.has_cond else None))
            self.mid_attn   = Residual(PreNorm(mid_dim, Attention(mid_dim)))
            self.mid_block2 = ResnetBlock(mid_dim, mid_dim, time_emb_dim=time_dim, cond_emb_dim=(time_dim if self.has_cond else None))
            
            self.ups = nn.ModuleList([])
            for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
                is_last = ind == (len(in_out) - 1)
                self.ups.append(nn.ModuleList([
                    ResnetBlock(dim_out + dim_in, dim_out, time_emb_dim=time_dim, cond_emb_dim=(time_dim if self.has_cond else None)),
                    ResnetBlock(dim_out + dim_in, dim_out, time_emb_dim=time_dim, cond_emb_dim=(time_dim if self.has_cond else None)),
                    Residual(PreNorm(dim_out, LinearAttention(dim_out))),
                    Upsample(dim_out, dim_in) if not is_last else nn.Conv2d(dim_out, dim_in, 3, padding=1)
                ]))
            # ====== Multi-scale shallow supervision for UnetRes ======
            self.enable_ms_supervision = True          # 开关
            self.ms_scales = {0, 2}                    # 选择在哪些 up stage 做浅监督
            self.ms_heads = nn.ModuleDict()
            
            reversed_in_out = list(reversed(in_out))
            for up_idx, (dim_in, dim_out) in enumerate(reversed_in_out):
                # 注意：这里用 dim_in（对应“上采样后”的 x 的通道数）
                self.ms_heads[f'up_{up_idx}'] = nn.Conv2d(dim_in, self.out_dim, 1)

            
            # ===== [D] final_res_block_shared 也接收 cond_emb（可选但推荐）=====
            self.final_res_block_shared = ResnetBlock(dim * 2, dim, time_emb_dim=time_dim, cond_emb_dim=(time_dim if self.has_cond else None))
            self.head_x0    = nn.Conv2d(dim, self.out_dim, 1)
            self.head_noise = nn.Conv2d(dim, self.out_dim, 1)
            
            # ===== [E] 高频头里的 ResnetBlock 也传入 cond_emb（否则与 FiLM 断开）=====
            self.head_hf = nn.Sequential(
                ResnetBlock(dim, dim, time_emb_dim=time_dim, cond_emb_dim=(time_dim if self.has_cond else None)),
            ...
            self.unet0 = Unet(dim,
                              ...
                              condition=condition,
                              input_condition=input_condition)
    def _set_cond(self, cond_tensor: torch.Tensor):
        # cond_tensor 的 shape: [B, cond_in_ch, H, W]
        self._cached_cond_tensor = cond_tensor
    
    def _clear_cond(self):
        self._cached_cond_tensor = None
    ...
    def forward(self, x, time, x_self_cond=None):
        if self.share_encoder == 1:
            ...
            cond_vecs = None
            if self.has_cond:
                assert hasattr(self, "_cached_cond_tensor") and (self._cached_cond_tensor is not None), \
                    "UnetRes: 没有收到条件张量，请确保在调用 forward 之前先调用 model._set_cond(cond)"
                cond_feats = self.cond_encoder(self._cached_cond_tensor)  # len = len(dims), 每个 [B, C, H, W]
                # 将每层映射到 time_dim
                cond_vecs  = [proj(f) for proj, f in zip(self.to_cond_vec, cond_feats)]  # 每个 [B, time_dim]
    
            # === Encoder ===
            h = []
            for layer_idx, (block1, block2, attn, downsample) in enumerate(self.downs):
                cvec = cond_vecs[layer_idx] if self.has_cond else None
                x = block1(x, t, cvec); h.append(x)
                x = block2(x, t, cvec); x = attn(x); h.append(x)
                x = downsample(x)
    
            # === Bottleneck ===
            cvec_mid = cond_vecs[-1] if self.has_cond else None
            x = self.mid_block1(x, t, cvec_mid)
            x = self.mid_attn(x)
            x = self.mid_block2(x, t, cvec_mid)
    
            # === Decoder（带 skip）===
            ms_preds = {}
            for up_idx, (block1, block2, attn, upsample) in enumerate(self.ups):
                # 与 encoder 对齐：decoder 第 up_idx 层用 cond_vecs[-(up_idx+1)]
                cvec = cond_vecs[-(up_idx+1)] if self.has_cond else None
    
                x = torch.cat((x, h.pop()), dim=1); x = block1(x, t, cvec)
                x = torch.cat((x, h.pop()), dim=1); x = block2(x, t, cvec)
                x = attn(x)
                x = upsample(x)
    
                if self.enable_ms_supervision and (up_idx in self.ms_scales):
                    key = f'up_{up_idx}'
                    if key in self.ms_heads:
                        ms_preds[key] = self.ms_heads[key](x)
    
            # === Final high-res block（也用 cond_emb）===
            x = torch.cat((x, r), dim=1)
            # 使用底层（最高分辨率）的 cond_vec；与 cond_feats[0] 对齐
            cvec_final = cond_vecs[0] if self.has_cond else None
            feat = self.final_res_block_shared(x, t, cvec_final)
    
            out_x0    = self.head_x0(feat)
            out_noise = self.head_noise(feat)
    
            # 高频头内部的 ResnetBlock 也要传 cond_emb（但它在 Sequential 里）
            # 这里无法显式传参，所以 head_hf 的 ResnetBlock 已经在 __init__ 里接了 cond_emb_dim，
            # 我们用一个 trick：把 cond_emb "加" 到 time_emb 上（shape 一致），再喂入 head_hf
            if self.has_cond and cvec_final is not None:
                t_for_hf = t + cvec_final  # [B, time_dim]
            else:
                t_for_hf = t
            # 复用 final 的 t_for_hf 进入 head_hf 的第一个 ResnetBlock
            # 做法：把 head_hf 拆开用（见下一个小段落的替换）
    
            # 为了能把 t_for_hf 传进去，我们把 head_hf 的 forward 改成显式两步：
            hf_resblk: ResnetBlock = self.head_hf[0]
            hf_conv: nn.Conv2d = self.head_hf[1]
            hf_feat = hf_resblk(feat, t_for_hf, cvec_final)
            out_hf = hf_conv(hf_feat)
    
            return out_x0, out_noise, out_hf, ms_preds
...
class ResidualDiffusion(nn.Module):
    ...
    def model_predictions(self, x_input, x, t, x_input_condition=0, x_self_cond=None, clip_denoised=True):
        if not self.condition:
            x_in = x
            if hasattr(self.model, "_clear_cond"):
                self.model._clear_cond()
        else:
            if self.input_condition:
                x_in = torch.cat((x, x_input, x_input_condition), dim=1)
                # —— 把“纯条件”缓存给 UnetRes（用于 FiLM）——
                if hasattr(self.model, "_set_cond"):
                    self.model._set_cond(torch.cat((x_input, x_input_condition), dim=1))
            else:
                x_in = torch.cat((x, x_input), dim=1)
                if hasattr(self.model, "_set_cond"):
                    self.model._set_cond(x_input)
        ...
        if self.objective == 'pred_res_noise':
            ...
            # 新：模型返回四元组
            x0_base, pred_noise, x0_hf, _ = model_output
            x_start = x0_base + x0_hf
            x_start = maybe_clip(x_start)
    ...
    def p_losses(self, imgs, t, noise=None):
        ...
        if self.objective == 'pred_res_noise':
            ...
            # 解包四元组
            x0_base, noise_pred, x0_hf, ms_preds = model_out
        
            # ------- 主重建：x_hat = x0_base + x0_hf -------
            x_hat = x0_base + x0_hf
            target_main  = x_start
            target_noise = noise
        
            loss_main  = self.loss_fn(x_hat, target_main, reduction='none')
            loss_noise = self.loss_fn(noise_pred, target_noise, reduction='none')
        
            # ------- 高频标签：HP = GT - LP(GT) -------
            def box_blur(img, k=5):
                return F.avg_pool2d(img, kernel_size=k, stride=1, padding=k//2)
            x_lp = box_blur(target_main)
            x_hp = target_main - x_lp
            loss_hf = F.l1_loss(x0_hf, x_hp, reduction='none')
            ...
            loss_grad = F.l1_loss(sobel_grad(x_hat), sobel_grad(target_main), reduction='none')
        
            # ------- 多尺度浅监督：每个尺度先做 mean，再累加成标量 -------
            loss_ms = torch.tensor(0., device=x_hat.device)  # 避免 Python float
            if isinstance(ms_preds, dict) and len(ms_preds) > 0:
                for name, ms_out in ms_preds.items():
                    gt_resized = F.interpolate(target_main, size=ms_out.shape[-2:], mode='bilinear', align_corners=False)
                    # 注意这里用 reduction='mean' 得到标量（或 0 维张量）
                    loss_ms = loss_ms + F.l1_loss(ms_out, gt_resized, reduction='mean')
        
            # ------- 汇总权重（可调） -------
            w_main, w_hf, w_grad, w_noise, w_ms = 1.0, 0.5, 0.2, 0.1, 0.2
            # 这里的四个是 B×C×H×W，loss_ms 是标量；标量会广播，不再冲突
            loss = (w_main*loss_main + w_hf*loss_hf + w_grad*loss_grad + w_noise*loss_noise)
            # 把标量 loss_ms 加进去
            loss = loss + w_ms*loss_ms
            ...
            target.append(x_start + noise)  # 按你原来的定义
